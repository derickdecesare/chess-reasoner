...
def main():
    ...
    dataset_rel_path = os.path.join("src","data", "chess", "chess_truncated_pgns_with_fen_100k.parquet")
    ...
    for idx, row in sample_df.iterrows():
        is_valid = verify_position(row['truncated_pgn'], row['fen'])
    ...
    for idx, row in df.head(3).iterrows():
        print(f"\nExample {idx + 1}:")
        print(f"Truncated PGN: {row['truncated_pgn']}")
        print(f"FEN: {row['fen']}")
        print(f"Splice point: {row['splice_point']}")
        print(f"Full moves: {row['full_moves']}")
        print("-" * 80)
